    ...
    <form id="casForm" action="{{ route('casToCache') }}" method="POST" onsubmit="return validateForm()">
        ...
        <div class="d-flex flex-column justify-content-center align-items-center mb-3">
            <div class="col-12">
                ...
                <input type="date" id="den" placeholder="Den" class="input mb-3" name="den" required
                    min="{{ now()->toDateString() }}">
            </div>
            <div class="col-12">
                <label for="od_kdy_hours">Od kdy</label><br>
                <select name="od_kdy_hours" id="od_kdy_hours" class="input mb-3" required>
                    @for ($hour = 6; $hour < 24; $hour++) @for ($minute=0; $minute < 60; $minute +=15) <option
            ...
            <div class="col-12">
                ...
                <select name="do_kdy_hours" id="do_kdy_hours" class="input mb-3" required>
                    ...
                    @for ($hour = 6; $hour < 24; $hour++) @for ($minute=0; $minute < 60; $minute +=15) <option
                        value="{{ str_pad($hour, 2, '0', STR_PAD_LEFT) }}:{{ str_pad($minute, 2, '0', STR_PAD_LEFT) }}">
                        {{ str_pad($hour, 2, '0', STR_PAD_LEFT) }}:{{ str_pad($minute, 2, '0', STR_PAD_LEFT) }}</option>
                        @endfor
                        @endfor
                </select>
            </div>
        ...
        <div class="d-flex justify-content-center align-items-center">
            <button type="button" class="btn btn-outline-dark tlacitko" onclick="window.location.href='/zakladni_udaje'"
